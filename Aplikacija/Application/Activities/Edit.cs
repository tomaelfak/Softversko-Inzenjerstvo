    ...
    public class Edit
    {
        ...
        public class Handler : IRequestHandler<Command, Result<Unit>>
        {
            ...
            public async Task<Result<Unit>> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                if (request.Activity.MaxParticipants < activity.NumOfParticipants)
                {
                    return Result<Unit>.Failure($"Max participants can't be less than current number of participants");
                }
                ...
                _mapper.Map(request.Activity, activity); // to je umesto da ispitujem sa ??= ja koristim automapper
